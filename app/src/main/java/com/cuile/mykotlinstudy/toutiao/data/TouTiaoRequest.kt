...
*
*/
class TouTiaoRequest(typeStr: String, private val touTiaoCallBack: TouTiaoCallBack) {
    ...
    init {
        completeUrl = "$BASE_URL?key=$APPKEY&type=$typeStr"
    }
    ...
    fun run() {
        async {
            ...
            if (toutiaoInfo.error_code == 0) {
                uiThread { touTiaoCallBack.requestSuccess(toutiaoInfo) }
            } else {
